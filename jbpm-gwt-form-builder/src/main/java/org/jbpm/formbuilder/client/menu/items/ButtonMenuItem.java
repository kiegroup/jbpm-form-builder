...
@Reflectable
public class ButtonMenuItem extends FBMenuItem {

    public ButtonMenuItem() {
        super();
    }

    public ButtonMenuItem(List<FBFormEffect> formEffects) {
        super(formEffects);
    }

    @Override
    public Label getDescription() {
        return new Label(CommonGlobals.getInstance().getI18n().MenuItemButton());
    }
    
    @Override
    protected ImageResource getIconUrl() {
        return FormBuilderResources.INSTANCE.button();
    }

    @Override
    public FBMenuItem cloneWidget() {
        return clone(new ButtonMenuItem());
    }

    @Override
    public FBFormItem buildWidget() {
        return build(new ButtonFormItem());
    }
}
